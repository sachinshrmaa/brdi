...
export default function AdminPage() {
  ...
  const filteredBookings = useMemo(() => {
    ...
    if (filterStatus === "checked-in") {
      ...
      filtered = filtered.filter((b) => !b.checked_in_at && !b.cancelled_at);
  ...
  async function login(event) {
    ...
    try {
      ...
      if (
        ...
        /Lock broken by another request/i.test(result.error?.message || "")
      ) {
      ...
      if (result.error) {
        if (
          ...
          setErrorMessage(
            "Login was interrupted by another open session. Close duplicate tabs and try again.",
          );
          return;
        }
  ...
  async function verifyToken() {
    ...
    const { data, error } = await supabase
      .from("bookings")
      .select(
        "id, applicant_name, appointment_at, checked_in_at, payment_status, amount, qr_token",
      )
    ...
    <section className="panel admin-page">
      <div className="header-row admin-header-row">
        <div>
          ...
          <p className="muted-text">
            Verify entries and manage all active bookings.
          </p>
        </div>
      ...
      <div className="admin-zones">
        <div className="admin-zone">
          <h3 className="admin-zone-title">Entry Gate Verification</h3>
          <p className="admin-zone-subtitle">
            Paste or scan QR token to validate and mark check-in.
          </p>
          ...
          {scanResult && (
            <div className="scan-result">
              <p>
                <strong>Booking:</strong> {scanResult.id}
              </p>
              <p>
                <strong>Name:</strong> {scanResult.applicant_name}
              </p>
              <p>
                <strong>Appointment:</strong>{" "}
                {new Date(scanResult.appointment_at).toLocaleString()}
              </p>
              <p>
                <strong>Payment:</strong> {scanResult.payment_status}
              </p>
              <p>
        ...
        <div className="admin-zone">
          <h3 className="admin-zone-title">
            All Bookings ({filteredBookings.length})
          </h3>
          <p className="admin-zone-subtitle">
            Use filters to focus on pending verification and operational
            decisions.
          </p>
            ...
            <table className="admin-table">
              ...
              <tbody>
                {filteredBookings.map((booking) => (
                  <tr
                    key={booking.id}
                    className={booking.cancelled_at ? "cancelled-row" : ""}
                  >
                    ...
                    <td data-label="Weight (t)">
                      {booking.estimated_weight_tons}
                    </td>
